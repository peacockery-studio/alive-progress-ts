...
describe('barFactory', () => {
  ...
  test('renders empty bar at 0%', () => {
    const factory = barFactory({ chars: '#', background: '-', borders: ['[', ']'] });
  ...
  test('shows overflow indicator', () => {
    const factory = barFactory({ chars: '#', background: '-', borders: ['[', ']'], errors: ['⚠', '✗'] });
    const bar = factory(12);
...
describe('bar with tip', () => {
  test('shows tip at progress edge', () => {
    const factory = barFactory({ chars: '=', tip: '>', background: ' ', borders: ['[', ']'] });
    const bar = factory(12);
